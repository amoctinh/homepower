...
if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
	...
	if ( ! function_exists( 'print_attribute_radio' ) ) {
		function print_attribute_radio( $checked_value, $value, $label, $name, $class ) {
			global $product;
			...
			printf( '<div class="%6$s"><input type="radio" name="%1$s" value="%2$s" id="%3$s" %4$s><label for="%3$s">%5$s</label></div>', $input_name, $esc_value, $id, $checked, $filtered_label, $class );
		}
	}
}
